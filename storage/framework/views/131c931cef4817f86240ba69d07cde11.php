...
(function () {
  ...
  const FINALIZADO_URL = <?php echo json_encode(route('nuevo-estudio.finalizado', ['estudio_id' => $estudio?->id]), 512) ?>;
  const MOSTRAR_FINALIZADO = false;

  let secs = 0, paused = false, fotos = <?php echo e($numCapturas); ?>;
  ...
  if (MOSTRAR_FINALIZADO) {
    ...
    const durSrvSecs = <?php echo json_encode($estudio?->duracion_segundos ?? 0, 15, 512) ?>;
    if (durSrvSecs > 0) {
  ...
  document.getElementById('sfDownloadVideo')?.addEventListener('click', () => {
    ...
    a.download = `estudio_<?php echo e($estudio?->folio ?? 'video'); ?>_${Date.now()}.${ext}`;
    a.click();
...
<?php /**PATH C:\Users\HP\enclaii-backend\resources\views\estudios\grabando\grabando-js.blade.php ENDPATH**/ ?>