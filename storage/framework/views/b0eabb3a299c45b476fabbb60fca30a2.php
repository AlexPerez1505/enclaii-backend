...
?>
<main class="wrap">
  <div class="brand"><b>ENCLA<span>II</span></b><p><?php echo e($clinicName); ?></p></div>
  <form class="card" method="POST" enctype="multipart/form-data" action="<?php echo e(route('qr.public.store', ['token' => $token])); ?>">
    <?php echo csrf_field(); ?>
    <h1>Pre-registro de paciente</h1>
    <p class="intro">Completa tus datos con calma. La clínica los revisará antes de crear tu expediente. El enlace vence el <?php echo e(format_user_date($expiresAt).' a las '.format_user_time($expiresAt)); ?>.</p>

    <?php if($patientMessage): ?>
      <div class="clinic-message">
        <strong>Mensaje de la clínica</strong>
        <?php echo e($patientMessage); ?>

      </div>
    <?php endif; ?>

    <?php if($errors->any()): ?>
      <div class="errors"><strong>Revisa la información:</strong><ul><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($error); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul></div>
    <?php endif; ?>

    <div class="grid">
      <div class="field wide"><label>Nombre completo <span class="req">*</span></label><input name="nombre_completo" value="<?php echo e(old('nombre_completo')); ?>" autocomplete="name" required></div>
      <div class="field"><label>Fecha de nacimiento <span class="req">*</span></label><input type="date" name="fecha_nacimiento" value="<?php echo e(old('fecha_nacimiento')); ?>" max="<?php echo e(now()->toDateString()); ?>" required></div>
      <div class="field"><label>Sexo <?php if($fieldRequired('sexo')): ?><span class="req">*</span><?php endif; ?></label><select name="sexo" <?php if($fieldRequired('sexo')): echo 'required'; endif; ?>><option value="">Seleccionar</option><option value="femenino" <?php if(old('sexo') === 'femenino'): echo 'selected'; endif; ?>>Femenino</option><option value="masculino" <?php if(old('sexo') === 'masculino'): echo 'selected'; endif; ?>>Masculino</option><option value="otro" <?php if(old('sexo') === 'otro'): echo 'selected'; endif; ?>>Otro</option></select></div>
      <div class="field"><label>Teléfono <span class="req">*</span></label><input type="tel" name="telefono" value="<?php echo e(old('telefono')); ?>" autocomplete="tel" required></div>
      <div class="field"><label>Correo electrónico <?php if($fieldRequired('email')): ?><span class="req">*</span><?php endif; ?></label><input type="email" name="email" value="<?php echo e(old('email')); ?>" autocomplete="email" <?php if($fieldRequired('email')): echo 'required'; endif; ?>></div>
      <div class="field"><label>Identificación <?php if($fieldRequired('identificacion')): ?><span class="req">*</span><?php endif; ?></label><input name="identificacion" value="<?php echo e(old('identificacion')); ?>" <?php if($fieldRequired('identificacion')): echo 'required'; endif; ?>></div>
      <div class="field"><label>Dirección <?php if($fieldRequired('direccion')): ?><span class="req">*</span><?php endif; ?></label><input name="direccion" value="<?php echo e(old('direccion')); ?>" autocomplete="street-address" <?php if($fieldRequired('direccion')): echo 'required'; endif; ?>></div>
      <div class="field"><label>Peso aproximado (kg) <?php if($fieldRequired('peso')): ?><span class="req">*</span><?php endif; ?></label><input type="number" step=".01" min="1" max="999.99" name="peso" value="<?php echo e(old('peso')); ?>" <?php if($fieldRequired('peso')): echo 'required'; endif; ?>></div>
      <div class="field"><label>Altura (metros) <?php if($fieldRequired('altura')): ?><span class="req">*</span><?php endif; ?></label><input type="number" step=".01" min=".3" max="2.99" name="altura" value="<?php echo e(old('altura')); ?>" <?php if($fieldRequired('altura')): echo 'required'; endif; ?>></div>
    </div>

    <?php if($showPhotoSection): ?>
      <div class="section">
        <h2>Foto del paciente <span style="color:var(--muted);font-weight:500">(<?php echo e($photoRequired ? 'obligatoria' : 'opcional'); ?>)</span> <?php if($photoRequired): ?><span class="req">*</span><?php endif; ?></h2>
    ...
    <div class="section">
      ...
      <div class="grid">
        <div class="field wide"><label>Procedimiento o estudio solicitado <?php if($fieldRequired('procedimiento')): ?><span class="req">*</span><?php endif; ?></label><input name="procedimiento" value="<?php echo e(old('procedimiento')); ?>" placeholder="Ej. Endoscopia digestiva alta" <?php if($fieldRequired('procedimiento')): echo 'required'; endif; ?>></div>
        <div class="field wide"><label>Motivo de consulta <?php if($fieldRequired('motivo_consulta')): ?><span class="req">*</span><?php endif; ?></label><textarea name="motivo_consulta" <?php if($fieldRequired('motivo_consulta')): echo 'required'; endif; ?>><?php echo e(old('motivo_consulta')); ?></textarea></div>
        <div class="field wide"><label>Alergias conocidas <?php if($fieldRequired('alergias')): ?><span class="req">*</span><?php endif; ?></label><textarea name="alergias" placeholder="Escribe “Ninguna” si no tienes alergias conocidas" <?php if($fieldRequired('alergias')): echo 'required'; endif; ?>><?php echo e(old('alergias')); ?></textarea></div>
        <div class="field wide"><label>Enfermedades o diagnósticos actuales <?php if($fieldRequired('enfermedades')): ?><span class="req">*</span><?php endif; ?></label><textarea name="enfermedades" <?php if($fieldRequired('enfermedades')): echo 'required'; endif; ?>><?php echo e(old('enfermedades')); ?></textarea></div>
        <div class="field wide"><label>Medicamentos que tomas actualmente <?php if($fieldRequired('medicamentos_actuales')): ?><span class="req">*</span><?php endif; ?></label><textarea name="medicamentos_actuales" <?php if($fieldRequired('medicamentos_actuales')): echo 'required'; endif; ?>><?php echo e(old('medicamentos_actuales')); ?></textarea></div>
        <div class="field wide"><label>Antecedentes médicos, cirugías u hospitalizaciones <?php if($fieldRequired('antecedentes_medicos')): ?><span class="req">*</span><?php endif; ?></label><textarea name="antecedentes_medicos" <?php if($fieldRequired('antecedentes_medicos')): echo 'required'; endif; ?>><?php echo e(old('antecedentes_medicos')); ?></textarea></div>
        <div class="field wide"><label>Observaciones adicionales <?php if($fieldRequired('observaciones')): ?><span class="req">*</span><?php endif; ?></label><textarea name="observaciones" <?php if($fieldRequired('observaciones')): echo 'required'; endif; ?>><?php echo e(old('observaciones')); ?></textarea></div>
      </div>
    </div>

    <div class="privacy">
      <input type="checkbox" id="privacyConsent" name="privacy_consent" value="1" <?php if(old('privacy_consent')): echo 'checked'; endif; ?> required>
      <label for="privacyConsent"><?php echo e($consentText); ?> <span class="req">*</span></label>
    </div>
...
<?php /**PATH C:\Users\LENOVO\enclaii-backend\resources\views/qr/public-form.blade.php ENDPATH**/ ?>