<script>
(function(){
  const caps = <?php echo json_encode($caps, 15, 512) ?>;
  const pacienteId = <?php echo json_encode((string) ($pacienteId ?? request('paciente', 1)), 512) ?>;
  let current = <?php echo e($current); ?>;
  const total  = caps.length;
  const csrfToken = <?php echo json_encode(csrf_token(), 15, 512) ?>;
  const imageSaveBaseUrl = <?php echo json_encode(url('/galeria/imagen'), 15, 512) ?>;
...
<?php /**PATH C:\Users\LENOVO\enclaii-backend\resources\views/galeria/verimagen/_scripts.blade.php ENDPATH**/ ?>