<?php $__env->startSection('title', 'Editar Video'); ?>
...
.ev-act-btn.wa{color:var(--green);border-color:rgba(61,220,151,.32);background:rgba(61,220,151,.07)}
...
<div class="rise d2">
  <div class="ev-topbar">
    ...
    <a href="<?php echo e(route('galeria.video', ['id' => $archivo->id, 'paciente' => $pacienteId])); ?>" class="ev-btn cancel">Cancelar</a>
    <a href="<?php echo e($pacienteId ? route('galeria.paciente', $pacienteId) : route('galeria')); ?>" class="ev-btn close" aria-label="Cerrar editor">
  ...
  <div class="ev-wrap">
    <div>
      <div class="ev-player-box" id="evPlayer">
        <video id="evVideoEl" class="ev-video" src="<?php echo e($videoUrl); ?>" preload="metadata" playsinline></video>
      ...
      <div class="ev-actions">
        ...
        <a class="ev-act-btn" href="<?php echo e($videoUrl); ?>" download="<?php echo e($downloadName); ?>"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>Exportar video</a>
        ...
        <a class="ev-act-btn wa" href="<?php echo e(route('mensajes', ['canal' => 'whatsapp', 'paciente' => $nombrePaciente, 'estudio' => $tipoEstudio, 'video' => $folioEstudio, 'diagnostico' => $estudio?->diagnostico])); ?>"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>Enviar por WhatsApp</a>
        <a class="ev-act-btn ia" href="<?php echo e(route('ia-reportes.generar', ['estudio' => $estudio?->id, 'paciente' => $pacienteId])); ?>"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 2a7 7 0 0 1 7 7c0 2.4-1.2 4.5-3 5.7V17a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2v-2.3C6.2 13.5 5 11.4 5 9a7 7 0 0 1 7-7z"/><line x1="9" y1="22" x2="15" y2="22"/></svg>IA Reportes</a>
      </div>
      ...
      <div>
        ...
        <div class="ev-caps-strip" id="evCapsStrip">
          <?php $__empty_1 = true; $__currentLoopData = $capturas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $cap): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <a class="ev-cap-item <?php echo e($i === 0 ? 'sel' : ''); ?>" href="<?php echo e(route('galeria.imagen', ['id' => $cap->id, 'paciente' => $pacienteId])); ?>">
              <div class="ev-cap-thumb">
                <img src="<?php echo e(media_url($cap->path)); ?>" alt="<?php echo e($cap->nombre_original ?? 'Captura'); ?>">
                <span class="ev-cap-num"><?php echo e($i + 1); ?></span>
              </div>
              <div class="ev-cap-ts"><?php echo e(format_user_time_with_seconds($cap->capturado_en)); ?></div>
            </a>
    ...
    <div class="ev-panel">
      ...
      <div class="ev-section">
        ...
        <div class="ev-adj-row"><span class="ev-adj-lbl">Brillo</span><input type="range" class="ev-slider" min="0" max="200" value="<?php echo e($editorConfig['brillo']); ?>" id="slBrillo"><span class="ev-adj-val" id="slBrilloVal"><?php echo e($editorConfig['brillo']); ?>%</span></div>
        <div class="ev-adj-row"><span class="ev-adj-lbl">Contraste</span><input type="range" class="ev-slider" min="0" max="200" value="<?php echo e($editorConfig['contraste']); ?>" id="slContraste"><span class="ev-adj-val" id="slContrasteVal"><?php echo e($editorConfig['contraste']); ?>%</span></div>
        <div class="ev-adj-row"><span class="ev-adj-lbl">Saturación</span><input type="range" class="ev-slider" min="0" max="200" value="<?php echo e($editorConfig['saturacion']); ?>" id="slSaturacion"><span class="ev-adj-val" id="slSaturacionVal"><?php echo e($editorConfig['saturacion']); ?>%</span></div>
        <div class="ev-adj-row"><span class="ev-adj-lbl">Nitidez</span><input type="range" class="ev-slider" min="0" max="100" value="<?php echo e($editorConfig['nitidez']); ?>" id="slNitidez"><span class="ev-adj-val" id="slNitidezVal"><?php echo e($editorConfig['nitidez']); ?>%</span></div>
        <div class="ev-zoom-row">
          <span class="ev-adj-lbl">Zoom</span>
          <button class="ev-zoom-btn" id="evZoomMinus" type="button">−</button>
          <span class="ev-zoom-val" id="evZoomVal"><?php echo e($editorConfig['zoom']); ?>%</span>
      ...
      <div class="ev-section">
        ...
        <div class="ev-field">
          ...
          <input class="ev-input" id="evNombre" value="<?php echo e($tituloVideo); ?>">
        </div>
        <div class="ev-field">
          <div class="ev-label">Hallazgos</div>
          <textarea class="ev-textarea" id="evHallazgos" rows="3"><?php echo e($estudio?->descripcion); ?></textarea>
        </div>
        <div class="ev-field">
          <div class="ev-label">Observaciones</div>
          <textarea class="ev-textarea" id="evObservaciones" rows="3"><?php echo e($estudio?->observaciones); ?></textarea>
        </div>
        <div class="ev-field">
          <div class="ev-label">Diagnóstico</div>
          <textarea class="ev-textarea" id="evDiagnostico" rows="2"><?php echo e($estudio?->diagnostico); ?></textarea>
        </div>
        <div class="ev-field">
          <div class="ev-label">Notas del archivo</div>
          <textarea class="ev-textarea" id="evDescripcion" rows="2"><?php echo e($archivo->descripcion); ?></textarea>
        </div>
      </div>
    </div>
  </div>
...
(function(){
  ...
  const updateUrl = <?php echo json_encode(route('galeria.video.update', $archivo->id), 512) ?>;
  const captureUrl = <?php echo json_encode(route('galeria.video.capture', $archivo->id), 512) ?>;
  const showUrl = <?php echo json_encode(route('galeria.video', ['id' => $archivo->id, 'paciente' => $pacienteId])) ?>;
  const settings = {
    brillo: Number(<?php echo json_encode($editorConfig['brillo'], 15, 512) ?>),
    contraste: Number(<?php echo json_encode($editorConfig['contraste'], 15, 512) ?>),
    saturacion: Number(<?php echo json_encode($editorConfig['saturacion'], 15, 512) ?>),
    nitidez: Number(<?php echo json_encode($editorConfig['nitidez'], 15, 512) ?>),
    zoom: Number(<?php echo json_encode($editorConfig['zoom'], 15, 512) ?>),
    rotacion: Number(<?php echo json_encode($editorConfig['rotacion'], 15, 512) ?>),
    flip_h: Boolean(<?php echo json_encode($editorConfig['flip_h'], 15, 512) ?>),
    flip_v: Boolean(<?php echo json_encode($editorConfig['flip_v'], 15, 512) ?>),
    trim_start: <?php echo json_encode($editorConfig['trim_start'], 15, 512) ?>,
    trim_end: <?php echo json_encode($editorConfig['trim_end'], 15, 512) ?>
  };
  ...
  async function captureFrame(){
    ...
    form.append('image', blob, `fotograma_video_<?php echo e($archivo->id); ?>_${Date.now()}.jpg`);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\enclaii-backend\resources\views\galeria\editarvideo.blade.php ENDPATH**/ ?>