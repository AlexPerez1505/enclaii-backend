...
(function(){
  window.__initBloqueos = function(EVENTS, MESES, DIAS_ES, buildDayFn) {
    ...
    blockSave.addEventListener('click', async () => {
      ...
      try {
        const resp = await fetch('<?php echo e(route('agenda.bloqueos.store')); ?>', {
          method: 'POST',
          headers: {
    ...
    blockConfirmYes.addEventListener('click', async () => {
      if (pendingDeleteId !== null) {
        ...
        try {
          const resp = await fetch('<?php echo e(route('agenda.bloqueos.destroy', ['bloqueo' => '__ID__'])); ?>'.replace('__ID__', pendingDeleteId), {
            method: 'DELETE',
            headers: {
              'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
              'X-Requested-With': 'XMLHttpRequest',
...
<?php /**PATH C:\Users\LENOVO\enclaii-backend\resources\views/agenda/_bloqueos.blade.php ENDPATH**/ ?>