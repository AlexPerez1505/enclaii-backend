...
<?php $__env->startSection('header-title', $pageTitle); ?>
      ...
      <div class="np-card rise d3">
        ...
        <div class="np-info-grid np-study-info-grid">
          <div class="np-info-box">
            ...
            <div class="np-field-value"><?php echo e($estudio->folio ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Procedimiento</label>
            <div class="np-field-value"><?php echo e($estudio->tipo ?? $paciente?->procedimiento ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Fecha del estudio</label>
            <div class="np-field-value"><?php echo e(format_user_date($estudio->fecha) ?: '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Estado</label>
            <div class="np-field-value"><?php echo e($estudio->estado_texto ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Médico</label>
            <div class="np-field-value"><?php echo e($estudio->medico ?? $paciente?->medico ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Sala</label>
            <div class="np-field-value"><?php echo e($estudio->sala ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Equipo</label>
            <div class="np-field-value"><?php echo e($estudio->equipo ?? '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Hora de inicio</label>
            <div class="np-field-value"><?php echo e(format_user_time($estudio->hora_inicio) ?: '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Hora de fin</label>
            <div class="np-field-value"><?php echo e(format_user_time($estudio->hora_fin) ?: '—'); ?></div>
          </div>
          <div class="np-info-box">
            <label>Duración</label>
            <div class="np-field-value"><?php echo e($duracionTexto); ?></div>
          </div>
          <div class="np-info-box">
            <label>Archivos</label>
            <div class="np-field-value"><?php echo e($galImagenes->count()); ?> imagen(es) / <?php echo e($galVideos->count()); ?> video(s)</div>
          </div>
          <div class="np-info-box">
            <label>Reportes</label>
            <div class="np-field-value"><?php echo e($reportes->count()); ?></div>
          </div>
          <div class="np-info-box np-wide">
            <label>Diagnóstico</label>
            <div class="np-field-value np-textarea-value"><?php echo e($estudio->diagnostico ?: '—'); ?></div>
          </div>
          <div class="np-info-box np-wide">
            <label>Descripción</label>
            <div class="np-field-value np-textarea-value"><?php echo e($estudio->descripcion ?: '—'); ?></div>
          </div>
          <div class="np-info-box np-wide">
            <label>Observaciones</label>
            <div class="np-field-value np-textarea-value"><?php echo e($estudio->observaciones ?: '—'); ?></div>
          </div>
        </div>
      </div>
    <?php endif; ?>
...
<div class="np-tab-panel" id="tab-galeria">
  ...
  <div class="pa-topbar rise d2">
    ...
    <button
      ...
      <?php if (! ($studyShareHasContent)): ?> disabled <?php endif; ?>
      title="<?php echo e($studyShareHasContent ? 'Enviar reportes, capturas y videos por correo' : 'Este estudio no tiene archivos ni reportes para enviar'); ?>"
    >
    ...
    <div>
      <section class="pa-hero">
        ...
        <div class="pa-stats">
          ...
          <?php if($isStudyDetail): ?>
          <div class="pa-stat"><strong><?php echo e($reportes->count()); ?></strong><span>Reportes</span></div>
          <?php endif; ?>
        </div>
      </section>
      ...
      <section class="pa-section">
        <div class="pa-section-head">
          ...
          <span class="pa-section-count"><?php echo e($reportes->count().' reportes'); ?></span>
        </div>
        <?php if($reportes->isNotEmpty()): ?>
        <div class="pa-report-list">
          <?php $__currentLoopData = $reportes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reporte): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            ?>
            <article class="pa-report-card" data-kind="reporte" data-title="<?php echo e(strtolower($reportPreview ?: 'reporte')); ?>">
              ...
              <div>
                <div class="pa-report-title">Reporte #<?php echo e($reporte->id); ?></div>
                <div class="pa-report-preview"><?php echo e(\Illuminate\Support\Str::limit($reportPreview ?: 'Sin contenido registrado.', 180)); ?></div>
                <?php if($reporte->contiene_hallazgos_criticos): ?>
                <span class="pa-report-badge">Hallazgos críticos</span>
                <?php endif; ?>
              </div>
              <div class="pa-report-date">
                <div><?php echo e(format_user_date($reporte->created_at) ?: 'Sin fecha'); ?></div>
                <a class="pa-btn primary" href="<?php echo e(route('ia-reportes.ver', ['reporte' => $reporte->id])); ?>" style="margin-top:8px;min-width:92px">Ver reporte</a>
              </div>
            </article>
...
<div class="np-tab-panel" id="tab-reportes">
  ...
  <div class="rpt-toolbar rise d1">
    ...
    <div class="rpt-toolbar-right">
      ...
      <a class="rpt-act-btn primary" href="<?php echo e(route('ia-reportes.pdf', $rpt)); ?>">
...
<div class="ns-modal-backdrop" id="studyMailBackdrop" data-send-url="<?php echo e(route('nuevo-estudio.correo.send', $estudio)); ?>">
    ...
    <form id="studyMailForm">
      ...
      <div class="ns-modal-body">
        <div class="ns-summary">
          <span><?php echo e($reportes->count()); ?> reporte(s)</span>
          <span><?php echo e($galImagenes->count()); ?> captura(s)</span>
          <span><?php echo e($galVideos->count()); ?> video(s)</span>
        </div>

        <div class="ns-field">
          <label for="studyMailRecipients">Destinatarios</label>
          <input id="studyMailRecipients" name="recipients" type="text" value="<?php echo e($paciente?->email); ?>" placeholder="correo@ejemplo.com, otro@ejemplo.com" autocomplete="email" required>
        </div>

        <div class="ns-field">
          <label for="studyMailSubject">Asunto</label>
          <input id="studyMailSubject" name="subject" type="text" value="<?php echo e($studyShareSubject); ?>" required>
        </div>

        <div class="ns-field">
          <label for="studyMailMessage">Mensaje</label>
          <textarea id="studyMailMessage" name="message" required>Hola, te comparto el estudio de <?php echo e($galNombre); ?> con sus reportes, capturas y videos.</textarea>
        </div>
        ...
        <div class="ns-modal-actions">
          ...
          <button class="np-new-study-btn" type="submit" id="studyMailSubmit" <?php if (! ($studyShareHasContent)): ?> disabled <?php endif; ?>>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <rect x="3" y="5" width="18" height="14" rx="2"/>
              <path d="m3 7 9 6 9-6"/>
            </svg>
    ...
    try {
      ...
      const response = await fetch("<?php echo e(route('capture.pairing-code.store')); ?>", {
        ...
        body: JSON.stringify({
          ...
          estudio_id: <?php echo json_encode($estudio?->id, 15, 512) ?>
        })
      });
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\LENOVO\enclaii-backend\resources\views/estudios/crear.blade.php ENDPATH**/ ?>