...
/* ====== MÓVIL / TABLET: solo el login, todo en una pantalla ====== */
@media (max-width:1024px){
  ...
  .ec-brand{margin-bottom:16px;gap:7px}
  ...
  .ec-subtitle{font-size:13.5px;margin-top:4px;margin-bottom:18px}
  .ec-field{margin-bottom:14px}
  .ec-label{font-size:13px;margin-bottom:6px}
  .ec-input{height:48px;font-size:16px} /* 16px evita el zoom automático de iOS */
  .ec-row{margin-bottom:16px;font-size:13px}
  .ec-btn{height:52px}
  .ec-footer{margin-top:16px;font-size:13.5px}
}

/* Pantallas muy bajitas (teléfonos chicos en horizontal o con barra grande) */
@media (max-width:1024px) and (max-height:640px){
  .ec-brand{margin-bottom:10px}
...
.ec-check input:checked{background:var(--blue-500);border-color:var(--blue-500)}
.ec-check input:checked::after{transform:scale(1)}
...
/* OJO: @csrf inserta un input oculto como PRIMER hijo del form,
   por eso los delays empiezan en nth-child(2). */
.ec-stagger:nth-child(2){animation-delay:0ms}
.ec-stagger:nth-child(3){animation-delay:55ms}
.ec-stagger:nth-child(4){animation-delay:110ms}
.ec-stagger:nth-child(5){animation-delay:165ms}
.ec-stagger:nth-child(6){animation-delay:220ms}
.ec-stagger:nth-child(7){animation-delay:275ms}
.ec-stagger:nth-child(8){animation-delay:330ms}
@keyframes ec-rise{to{opacity:1;transform:translateY(0)}}

/* Shake en error de credenciales */
@keyframes ec-shake{
...
<div class="ec-shell">
  ...
  <main class="ec-form-panel">
    <form method="POST" action="{{ route('login') }}" id="loginForm" novalidate
          class="@if($errors->has('email') || $errors->has('password')) ec-shake @endif">
      @csrf
      ...
      <div class="ec-stagger">
        ...
        <p class="ec-subtitle">Accede a tu plataforma de endoscopia inteligente.</p>
      </div>
      ...
      <div class="ec-field ec-stagger">
        ...
        <div class="ec-input-wrap">
          ...
          <button type="button" class="ec-eye" id="togglePass" aria-label="Mostrar contraseña" aria-pressed="false">
      ...
      @if (Route::has('register'))
        <p class="ec-footer ec-stagger">
          ¿No tienes cuenta? <a class="ec-link" href="{{ route('register') }}">Regístrate ahora</a>
        </p>
      @endif
    </form>
  </main>
  ...
  <aside class="ec-visual" aria-hidden="true">
    ...
    <div class="ec-scene" id="scene">
      ...
      {{-- Lottie central: guarda tu animación en public/animations/medico.json --}}
      <lottie-player
        class="ec-lottie"
