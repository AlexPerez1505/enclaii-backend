...
@section('header-sub', 'Consulta y administra imágenes y videos de estudios')
...
<div class="gp-workspace rise d2" id="gpWorkspace">
  <section class="gp-shell">
    ...
    <div class="gp-list" id="gpList">
      @foreach($pacientes as $p)
        <a href="{{ route('galeria.paciente', $p['id']) }}"
           ...
           data-code="{{ mb_strtolower($p['codigo']) }}"
           data-phone="{{ mb_strtolower($p['telefono']) }}"
           data-studies="{{ rawurlencode($p['filtros']->toJson()) }}">
          ...
          <div class="gp-main">
            ...
            <div class="gp-id">
              ID: {{ $p['codigo'] }} <span style="margin:0 8px">•</span>
              {{ $p['edad'] }} <span style="margin:0 8px">•</span> {{ $p['sexo'] }}
            </div>
            <div class="gp-meta">
    ...
    <div class="gp-footer">
      <span id="gpResultCount">{{ $pacientes->count() }} paciente(s) encontrado(s)</span>
