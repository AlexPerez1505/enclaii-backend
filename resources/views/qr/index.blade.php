    ...
    <div class="qr-card">
      ...
      <div class="qr-builder">
        <div class="qr-preview qr-print-target">
          <div class="qr-preview-box {{ $currentAvailable ? '' : 'unavailable' }}">
            @if($currentLink)
              <img src="{{ route('qr.links.image', $currentLink) }}" alt="Código QR {{ $currentCode }}">
              @unless($currentAvailable)
                <span class="qr-preview-state">{{ $statusLabels[$currentStatus] ?? 'No disponible' }}</span>
              @endunless
            @else
          ...
          @elseif($currentLink)
            <span class="qr-status {{ $currentStatus }}">{{ $statusLabels[$currentStatus] ?? ucfirst($currentStatus) }}</span>
          @endif
        </div>
        ...
        <form class="qr-create" method="POST" action="{{ route('qr.links.store') }}">
          ...
          <fieldset>
            ...
            <div class="qr-expiration">
              <input type="radio" id="qr24" name="expires_in_hours" value="24" @checked($qrDefaultExpiration === '24')>
              <label for="qr24">○ 24 horas</label>
              <input type="radio" id="qr48" name="expires_in_hours" value="48" @checked($qrDefaultExpiration === '48')>
              <label for="qr48">○ 48 horas</label>
              <input type="radio" id="qr168" name="expires_in_hours" value="168" @checked($qrDefaultExpiration === '168')>
          ...
          <div>
            ...
            <div class="qr-message-wrap">
              <textarea class="qr-message" id="qrPatientMessage" name="patient_message" maxlength="150" placeholder="Por favor completa tus datos con la mayor información posible. Gracias.">{{ $qrDefaultPatientMessage }}</textarea>
              <span class="qr-message-count"><span id="qrMessageCount">{{ mb_strlen($qrDefaultPatientMessage) }}</span>/150</span>
            </div>
          </div>
          <button class="qr-primary" type="submit"><span style="font-size:16px">＋</span> {{ $currentLink && !$currentAvailable ? 'Generar reemplazo' : 'Generar nuevo código' }}</button>
        </form>
      </div>
      ...
      @if($currentLink)
        <div class="qr-current-info">
          <div class="qr-code-line">
            <span>Código: <strong>{{ $currentCode }}</strong></span>
            <button class="qr-icon-btn" type="button" title="Copiar código" data-copy-text="{{ $currentCode }}">▣</button>
          </div>
          <div class="qr-dates">
            <span><b>Creado:</b> {{ format_user_date($currentLink->created_at).' · '.format_user_time($currentLink->created_at) }}</span>
            <span class="expires"><b>Expira:</b> {{ format_user_date($currentLink->expires_at).' · '.format_user_time($currentLink->expires_at) }}</span>
          </div>
        </div>
      ...
      <section class="qr-history">
        ...
        <div class="qr-history-tabs" role="tablist" aria-label="Filtrar historial de QR">
          <button class="qr-history-tab {{ $defaultHistoryStatus === 'active' ? 'active' : '' }}" type="button" data-history-filter="active">Activos <span>{{ $activeLinks->count() }}</span></button>
          <button class="qr-history-tab {{ $defaultHistoryStatus === 'submitted' ? 'active' : '' }}" type="button" data-history-filter="submitted">Utilizados <span>{{ $usedLinks->count() }}</span></button>
          <button class="qr-history-tab {{ $defaultHistoryStatus === 'expired' ? 'active' : '' }}" type="button" data-history-filter="expired">Expirados <span>{{ $expiredLinks->count() }}</span></button>
          <button class="qr-history-tab {{ $defaultHistoryStatus === 'revoked' ? 'active' : '' }}" type="button" data-history-filter="revoked">Cancelados <span>{{ $revokedLinks->count() }}</span></button>
        </div>
          ...
          <table class="qr-history-table">
            ...
            <tbody>
              @forelse($links as $link)
                ...
                @endphp
                <tr data-history-status="{{ $displayStatus }}" @if($displayStatus !== $defaultHistoryStatus) hidden @endif>
                  <td><a class="qr-history-code" href="{{ route('qr.index', ['qr' => $link->id]) }}">{{ $code }}</a></td>
                  <td>{{ $validityLabel }}</td>
                  <td>{{ format_user_date($link->created_at) }}</td>
                  <td>{{ format_user_date($link->expires_at) }}</td>
                  <td><span class="qr-status {{ $displayStatus }}">{{ $statusLabels[$displayStatus] ?? ucfirst($displayStatus) }}</span></td>
                  <td>{{ $link->preregistration ? 1 : 0 }}</td>
                  <td>
                    <details class="qr-history-menu">
                      <summary aria-label="Acciones del código">⋮</summary>
                      <div class="qr-history-menu-pop">
                        <a href="{{ route('qr.index', ['qr' => $link->id]) }}">Ver código</a>
