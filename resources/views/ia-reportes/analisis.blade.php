...
@section('header-title', 'Análisis predictivo completo')
@section('header-sub')
...
@section('content')
  ...
  <div class="an-top">
    <a class="an-back" href="{{ route('ia-reportes') }}">
    ...
    <article class="card rise d2">
      <div class="an-pat">
        ...
        <div>
          ...
          <div class="mt">{{ $p['study'] }} · {{ $p['date'] }}</div>
        </div>
      </div>
      ...
      <div class="an-hero">
        <div class="an-gauge">
          <svg viewBox="0 0 120 120">
            ...
            <circle class="val" cx="60" cy="60" r="50" stroke-dasharray="314.16" stroke-dashoffset="314.16" data-off="{{ $off }}"/>
          </svg>
          <div class="ctr">
            <div>
              <b>{{ $p['prob'] }}%</b><br>
        ...
        <div class="an-hero-info">
          ...
          <span class="an-risk" style="color:{{ $p['color'] }}">
          ...
          <div class="mt" style="font-size:12.5px;color:var(--txt-soft);margin-top:8px">{{ $p['sub'] }}</div>
        </div>
      </div>
      ...
      <div class="an-sect">
        ...
        @foreach($p['factors'] as $f)
          <div class="factor">
            <div class="top"><span>{{ $f[0] }}</span><b>{{ $f[1] }}%</b></div>
            <div class="fbar"><i data-w="{{ $f[1] }}"></i></div>
          </div>
        @endforeach
      </div>
      <div class="an-note">
  ...
  <article class="card rise d4" style="margin-top:16px">
    <div class="an-sect" style="margin-top:0">
      <h3>ANÁLISIS POR IMAGEN ({{ count($p['images']) }})</h3>
      <p style="font-size:12.5px;color:var(--txt-soft);margin:-8px 0 14px">Qué detectó la IA en cada imagen y por qué sustenta el diagnóstico de {{ $p['cond'] }}.</p>
      @foreach($p['images'] as $img)
        <div class="img-card">
          <div class="img-thumb {{ $img[0] }}">
            <span class="ai-tag">IA</span>
            <span class="ai-box"></span>
          </div>
          <div class="img-body">
            <div class="ihead">
              <span class="iregion">{{ $img[1] }}</span>
              <span class="iconf">{{ $img[4] }}% relevancia</span>
            </div>
            <div class="idet">{{ $img[2] }}</div>
            <div class="iwhy"><b>Por qué importa:</b> {{ $img[3] }}</div>
          </div>
        </div>
      @endforeach
    </div>
  ...
  <article class="card an-generate rise d5">
    ...
    <a class="an-gen-btn" href="{{ route('ia-reportes.generar') }}">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a7 7 0 0 1 7 7c0 2.4-1.2 4.5-3 5.7V17a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2v-2.3C6.2 13.5 5 11.4 5 9a7 7 0 0 1 7-7z"/><line x1="9" y1="22" x2="15" y2="22"/></svg>
