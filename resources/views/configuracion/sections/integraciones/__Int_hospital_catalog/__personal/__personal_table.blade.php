...
<div class="cat-table-wrap">
  <table class="cat-table">
    ...
    <tbody>
      @forelse($clinicMembers as $member)
        @php
        ...
        <tr>
          <td class="cat-name">
            {{ $member->name }}
            @if($catIsCurrentUser)<span class="cat-you">Tú</span>@endif
          </td>
          <td class="cat-soft">{{ $catSpecialty }}</td>
          <td class="cat-role">{{ $catRoleLabels[$member->clinica_rol] ?? ucfirst($member->clinica_rol) }}</td>
          <td class="cat-soft">{{ $member->email }}</td>
          <td><span class="cat-badge cat-badge-on">Activo</span></td>
          <td class="cat-actions">
            @if($catCanRemove)
              <button type="button" class="cat-del-btn cat-member-remove" data-action="{{ route('configuracion.clinic-members.destroy', $member) }}" data-member-name="{{ $member->name }}">
            ...
            @else
              ...
              @endphp
              <span class="cat-no-action" title="{{ $catNoActionTitle }}">—</span>
            @endif
          </td>
        </tr>
      @empty
      ...
      @foreach($clinicInvitations ?? [] as $invitation)
        <tr>
          <td class="cat-name cat-pending-name">{{ $invitation->email }}</td>
          <td class="cat-soft">—</td>
          <td class="cat-role">{{ $catRoleLabels[$invitation->rol] ?? ucfirst($invitation->rol) }}</td>
          <td class="cat-soft">{{ $invitation->email }}</td>
          <td><span class="cat-badge cat-badge-off">Invitado</span></td>
          <td class="cat-actions">
            @if($catIsClinicOwner)
              <button type="button" class="cat-del-btn cat-invite-revoke" data-action="{{ route('configuracion.clinic-invitations.destroy', $invitation) }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                </svg>
              </button>
