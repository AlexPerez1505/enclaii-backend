    ...
    <div class="qrset-stack">
      <article class="card rise d2">
        ...
        <div class="qrset-control">
          ...
          <textarea class="qrset-textarea" id="cfgQrMessage" data-setting="qr_default_patient_message" maxlength="150">{{ $qrSettings['qr_default_patient_message'] ?? '' }}</textarea>
          <div class="qrset-count"><span data-count-for="cfgQrMessage">{{ mb_strlen($qrSettings['qr_default_patient_message'] ?? '') }}</span>/150</div>
        </div>

        <div class="qrset-control">
          <label for="cfgQrWhatsapp">Plantilla para WhatsApp</label>
          <textarea class="qrset-textarea" id="cfgQrWhatsapp" data-setting="qr_whatsapp_template" maxlength="500">{{ $qrSettings['qr_whatsapp_template'] ?? '' }}</textarea>
      ...
      <article class="card rise d3">
        ...
        <div class="qrset-control">
          ...
          <textarea class="qrset-textarea tall" id="cfgQrConsent" data-setting="qr_consent_text" maxlength="700">{{ $qrSettings['qr_consent_text'] ?? '' }}</textarea>
        ...
        <div class="qrset-control">
          ...
          <div class="qrset-fields">
            @foreach($qrFieldOptions as $fieldKey => [$fieldLabel, $fieldHelp])
              <label class="qrset-check">
                <input type="checkbox" value="{{ $fieldKey }}" data-setting-list="qr_required_fields" @checked($qrRequiredFields->contains($fieldKey))>
                <span><b>{{ $fieldLabel }}</b><span>{{ $fieldHelp }}</span></span>
              </label>
            @endforeach
          </div>
        </div>
...
<script>
(function(){
  const clinicName = @json(auth()->user()->clinica?->nombre ?? 'la clínica');
