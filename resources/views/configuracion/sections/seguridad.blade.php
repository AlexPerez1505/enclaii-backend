...
<div class="cfg-panel" data-panel="seguridad">
  ...
  <div class="sec-top">
    ...
    <article class="card rise d3">
      ...
      <form class="sec-search" method="GET" action="{{ route('configuracion') }}">
        <input type="hidden" name="tab" value="seguridad">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <input type="search" name="activity_search" value="{{ request('activity_search') }}" placeholder="Buscar acción, categoría o IP...">
      ...
      @if($activityLogs->hasPages())
        <nav class="sec-pagination" aria-label="Paginación del registro de actividad">
          ...
          @else
            <a href="{{ $activityLogs->previousPageUrl() }}">Anterior</a>
          @endif
          <span>Página {{ $activityLogs->currentPage() }} de {{ $activityLogs->lastPage() }}</span>
          @if($activityLogs->hasMorePages())
            <a href="{{ $activityLogs->nextPageUrl() }}">Siguiente</a>
          @else
  ...
  <div class="sec-pw-overlay" id="secPasswordModal" aria-hidden="true">
    <form
      class="sec-pw-modal"
      id="secPasswordForm"
      data-update-url="{{ route('configuracion.password.update') }}"
    >
      @csrf
      <div class="sec-pw-head">
        <div>
