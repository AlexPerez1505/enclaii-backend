...
<div class="widget widget-minimal mode-hidden d1" data-widget-id="next-patient-min" data-w="3">
  ...
  <div class="card card-minimal card-minimal-next" style="overflow:hidden">
    ...
    <a class="min-icon" href="{{ $urlMinCita }}" style="flex:1 1 60%;width:100%;min-height:0;display:grid;place-items:center;color:var(--cyan);text-decoration:none;cursor:pointer">
    ...
    <div class="min-text" style="flex:0 0 22%;display:flex;flex-direction:column;justify-content:center;gap:0.35em;text-align:center;min-height:0">
      <div class="min-value" style="font-size:clamp(0.85em,4.5cqi,1.35em);line-height:1.1">{{ $nombreMinCita }}</div>
      @if ($proximaCita)
        <div class="min-meta" style="font-size:clamp(0.65em,3cqi,0.9em)">{{ $horaMinCita }} · {{ $procMinCita }}</div>
      @else
