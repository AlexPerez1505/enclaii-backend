  ...
  <article class="card card-minimal card-minimal-list" style="overflow:hidden">
    ...
    <ul class="min-list" style="flex:1;min-height:0;overflow-y:auto;display:flex;flex-direction:column;gap:0.5em">
      @forelse (($pendientesHoy ?? []) as $cita)
        ...
        @endphp
        <li style="flex:none">
          <a class="min-list-row" href="{{ $urlList }}" style="display:flex;align-items:center;gap:0.65em;text-decoration:none;color:inherit">
            ...
            <span class="min-list-time" style="flex:none;font-size:clamp(0.7em,3cqi,0.9em);color:var(--txt-soft)">{{ $horaList }}</span>
            <span class="min-list-name" style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:clamp(0.8em,3.5cqi,1em);font-weight:600">{{ $nombreList }}</span>
            <span class="min-list-proc" style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:clamp(0.7em,3cqi,0.9em);color:var(--txt-soft)">{{ $procList }}</span>
    ...
    <a class="min-btn" href="{{ route('agenda') }}" style="flex:0 0 auto">
      Ver agenda
